...
```
$$\mathrm{Adv}_{\mathrm{PRG}}[A, G] = \left| \Pr_{k \xleftarrow{R} K}[A(G(k)) = 1] - \Pr_{r \xleftarrow{R} \{0,1\}^n}[A(r) = 1] \right|$$
```
...
```
接近 1 意味着区分器 $A$ 能够以极大概率将 $G$ 生成的伪随机序列与真正的均匀随机序列区分开，这表示该 PRG 是不安全的；接近 0 意味着区分器 $A$ 几乎无法区分 $G$ 的输出与真随机序列，这表示针对测试 $A$，该 PRG 表现出良好的伪随机属性（即是安全的）。
```
...
```
如果 $A$ 恒输出 0，则 $\Pr[A(G(k)) = 1] = 0$ 且 $\Pr[A(r) = 1] = 0$。因此优势为：$|0 - 0| = 0$。
```
...
```
根据题意，$G(k)$ 有 $\frac{2}{3}$ 的概率其最高有效位（MSB）为 1。因此：$$\Pr[A(G(k)) = 1] = \frac{2}{3}$$
```
...
```
对于真正的均匀随机字符串 $r$，其最高有效位为 1 的概率恰好是一半。因此：$$\Pr[A(r) = 1] = \frac{1}{2}$$
```
...
```
$$\mathrm{Adv}_{\mathrm{PRG}}[A, G] = \left| \frac{2}{3} - \frac{1}{2} \right| = \left| \frac{4}{6} - \frac{3}{6} \right| = \frac{1}{6}$$
```
...
```
不安全。因为存在一个高效运行（仅需检查最高位）的统计算法 $A$，它使得 $\mathrm{Adv}_{\mathrm{PRG}}[A, G] = \frac{1}{6}$。该优势是一个显著的非忽略（non-negligible）常数，违反了安全 PRG “对于任何有效测试 $A$，优势必须可忽略” 的定义。
```
...
```
对于任意给定的密文 $c$ 以及任意两个不同的明文 $m_0, m_1 \in \mathcal{M}$，满足：$$\Pr_k[E(k, m_0) = c] = \Pr_k[E(k, m_1) = c]$$等价地，$\Pr_k[E(k, m) = c]$ 的值仅取决于密文空间的大小，而与具体的明文 $m$ 无关（即恒等于一个常数 $\frac{1}{|\mathcal{C}|}$）。
```
...
```
恰好存在 1 个密钥。因为在 OTP 中 $E(k, m) = m \oplus k$。若要使得 $m \oplus k = c$，根据异或运算的性质，两边同时异或 $m$，即可唯一解得 $k = m \oplus c$。对于固定的 $m$ 和 $c$，异或运算是确定的且一一对应的。
```
...
```
在 OTP 中，密钥 $k$ 是从密钥空间 $\mathcal{K} = \{0,1\}^n$ 中均匀随机抽取的，且 $|\mathcal{K}| = 2^n$。
根据 (b) 的结论，对于任意固定的明文 $m$ 和密文 $c$，恰好存在唯一的一个密钥 $k = m \oplus c$ 能够满足 $E(k, m) = c$。
因此，这个特定密钥被选中的概率为 $\Pr_k[E(k, m) = c] = \frac{1}{|\mathcal{K}|} = \frac{1}{2^n}$。
由于这个概率值 $\frac{1}{2^n}$ 仅与密钥长度 $n$ 有关，对所有的 $m \in \mathcal{M}$ 都恒等，故满足 (a) 中完美保密性的定义：$\Pr_k[E(k, m_0) = c] = \Pr_k[E(k, m_1) = c] = \frac{1}{2^n}$。证明完毕。
```
...
```
语义安全（Semantic Security）的核心要求是：给定密文 $c$，任何多项式时间攻击者都无法推断出关于明文 $m$ 的任何有意义的偏部信息（除了长度等公开信息）。由于算法 $A$ 能够高效且确定地提取明文的特定位 $\mathrm{LSB}(m)$，这就证明了密文 $c$ 泄露了明文的实质性信息，直接打破了语义安全性的定义。
```
...
```
$B$ 选择两条长度相同但 $\mathrm{LSB}$ 不同的消息：例如选择 $m_0 = 0^n$（其 $\mathrm{LSB}$ 为 0）和 $m_1 = 0^{n-1}1$（其 $\mathrm{LSB}$ 为 1）。$B$ 收到挑战密文 $c$ 后，将 $c$ 输入子程序 $A$。由于 $A(c)$ 总是能正确计算出原明文的 LSB，如果 $A(c)$ 输出 0，说明原明文的最低位是 0，$B$ 猜测挑战者加密的是 $m_0$（输出猜测 $b' = 0$）；如果 $A(c)$ 输出 1，说明最低位是 1，$B$ 猜测挑战者加密的是 $m_1$（输出猜测 $b' = 1$）。
```
...
```
优势计算公式为 $\mathrm{Adv}_{\mathrm{SS}}[B, \mathbb{E}] = \left| \Pr[B(c_0)=1] - \Pr[B(c_1)=1] \right|$。
当挑战者加密 $m_0$ 时（即 $b=0$），密文为 $c_0$，$A(c_0)$ 将输出 0，$B$ 会输出 0。因此 $\Pr[B(c_0)=1] = 0$。
当挑战者加密 $m_1$ 时（即 $b=1$），密文为 $c_1$，$A(c_1)$ 将输出 1，$B$ 会输出 1。因此 $\Pr[B(c_1)=1] = 1$。
故 $\mathrm{Adv}_{\mathrm{SS}}[B, \mathbb{E}] = |0 - 1| = 1$。
```
...
```
因为 $k$ 在 $\{0,1\}^n$ 上是独立且均匀分布的，而 $m_0$ 是一个常数。根据第 14 题中提到的“异或运算的性质（Shannon 定理）”：任何一个均匀随机变量（$k$）与另一个常数（$m_0$）进行异或，其结果分布不变。因此 $c = k \oplus m_0$ 在 $\{0,1\}^n$ 上仍是均匀分布的。
```
...
```
同理，由于 $k$ 在 $\{0,1\}^n$ 上是均匀分布的，它与任何固定的常数字符串 $m_1$ 的异或结果 $c = k \oplus m_1$ 同样在 $\{0,1\}^n$ 上是均匀分布的。
```
...
```
由 (a) 和 (b) 的结论可知，在 EXP(0) 中密文是完美均匀分布的（任意 $x \in \{0,1\}^n$ 出现的概率均为 $\frac{1}{2^n}$），在 EXP(1) 中密文也是完美均匀分布的（任意 $x$ 的概率也是 $\frac{1}{2^n}$）。由于它们的概率质量函数完全一致，因此这两种实验中生成的挑战密文分布是完全相同的。
```
...
```
对于任何区分器/攻击者算法 $A$，它的输出概率仅依赖于其输入的概率分布。
由 (c) 可知，无论是输入 $k \oplus m_0$ 还是 $k \oplus m_1$，由于这两者作为输入给算法 $A$ 时遵循相同的均匀分布，算法 $A$ 的行为和输出分布也会完全一致。
这就意味着 $\Pr[A(k \oplus m_0) = 1]$ 必然严格等于 $\Pr[A(k \oplus m_1) = 1]$。
因此，二者之差为 0，即 $\mathrm{Adv}_{\mathrm{SS}}[A, \mathrm{OTP}] = 0$，证明了任何攻击者在语义安全游戏中都无法取得优势。
```
