...
**答案：** `6c73d5240a948c86981bc2948148`
...
```
接近 1：区分器A能以极高概率区分 PRG 输出和真随机串，说明 PRG不安全；
接近 0：区分器A无法区分 PRG 输出和真随机串，说明 PRG安全（优势越接近 0，安全性越高）。
```
...
```
Prk←K​[A(G(k))=1]=0,Prr←{0,1}n​[A(r)=1]=0
AdvPRG​[A,G]=∣0−0∣=0
```
...
```
Pr[A(G(k))=1]=Pr[MSB(G(k))=1]=43​=0.75
```
...
```
不安全。
原因：安全 PRG 要求对所有多项式时间区分器A，AdvPRG​[A,G]是可忽略的（趋近于 0）。
该 PRG 存在区分器A，优势为 0.25（非可忽略），因此不安全。
```
...
```
对于任意明文m∈M、密文c∈C，满足：
Pr[E(k,m)=c]=Pr[E(k,m′)=c],∀m′∈M
即：对任意密文c，所有明文映射到c的概率相等，攻击者无法从密文获取明文的任何信息。
```
...
```
OTP 加密公式：E(k,m)=m⊕k=c，根据异或可逆性，唯一满足条件的密钥是k=m⊕c。因此：恰好有 1 个密钥。
```
...
```
对任意明文m1​,m2​∈M、密文c∈C：
满足E(k,m1​)=c的密钥数为 1，因此Pr[E(k,m1​)=c]=∣K∣1​；
满足E(k,m2​)=c的密钥数为 1，因此Pr[E(k,m2​)=c]=∣K∣1​；


因此Pr[E(k,m1​)=c]=Pr[E(k,m2​)=c]，满足完美保密性的定义，故 OTP 具有完美保密性。
```
...
```
语义安全的定义：对任意多项式时间攻击者，无法从密文获取明文的任何非平凡信息。该方案中，攻击者可通过算法
A
直接获取明文的
LSB(m)
（非平凡信息），违反语义安全的定义，因此不是语义安全的。
```
...
```
挑战者生成密钥k；
攻击者B提交两个等长明文m0​,m1​，满足LSB(m0​)=0，LSB(m1​)=1；
挑战者随机选取b∈{0,1}，返回密文c=E(k,mb​)；
攻击者B调用A(c)得到LSB(mb​)：
若LSB(mb​)=0，则猜测b=0；
若LSB(mb​)=1，则猜测b=1；
...
```
Advss​[B,E]=∣Pr[B 猜对 b]−21​∣由于A总能正确得到LSB(mb​)，B猜对b的概率为1，因此：
Advss​[B,E]=∣1−21​∣=0.5
```
...
```
密钥k是{0,1}n上的独立均匀随机变量，根据异或的性质：任意变量与独立均匀随机变量异或，结果仍是均匀分布。
```
...
```
任意变量与独立均匀随机变量异或，结果仍是均匀分布。
密钥k是独立均匀随机变量，c=k⊕m1​满足异或均匀性定理，因此在{0,1}n上均匀分布。
```
...
```
EXP(0)和EXP(1)的密文均为{0,1}n上的均匀分布，因此两个实验的密文分布完全相同（identical distributions）。
```
...
```
Advss​[A,OTP]=∣Pr[A(⊕m0​)=1]−Pr[A(⊕m1​)=1]∣=0
由于两个实验的密文分布完全相同，攻击者A无法区分EXP(0)和EXP(1)的密文，因此：
Pr[A 猜对 b∣EXP(0)]=Pr[A 猜对 b∣EXP(1)]=21​因此：
Advss​[A,OTP]=∣Pr[A(c)=1∣c←EXP(0)]−Pr[A(c)=1∣c←EXP(1)]∣=0故 OTP 是语义安全的。
```
