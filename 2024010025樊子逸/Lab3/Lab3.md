...
**答案：** ____6c73d5240a948c86981bc2808548_______
...
```Adv接近1意味着A能以高概率区分G的输出和真随机序列，G不安全

```Adv接近0意味着A无法有效区分，G对该测试是安全的
...
```不安全，因为优势为1/6不可忽略，存在有效的统计测试能够区分G的输出与真随机序列。
...
```对于任意明文m∈M和密文c∈C，有 Pr_k[E(k,m)=c] = 1/|K|，即该概率与m无关。
...
```对于OTP，E(k,m)=m⊕k。给定m和c，方程m⊕k=c有唯一解k=m⊕c，故恰好存在1个密钥。
...
```由于密钥k均匀随机取自K，故Pr_k[E(k,m)=c] = 1/|K|，与m无关，因此OTP满足完美保密性。
...
```若存在算法A总能从密文c中恢复LSB(m)，则考虑语义安全游戏：攻击者B选择两条长度相同且LSB不同的消息m0（LSB=0）和m1（LSB=1），发送给挑战者。挑战者随机选择b并返回c=E(k,m_b)。B调用A(c)得到LSB的猜测，若为0则输出0，否则输出1。由于A总是正确，B能以概率1猜中b，因此方案不是语义安全的。
...
```B选择m0和m1，其中m0和m1除最低有效位外完全相同，且LSB(m0)=0，LSB(m1)=1。收到密文c后，B计算b' = A(c)，若b'=0则猜测b=0，否则猜测b=1。
...
```Adv_SS[B, E] = |Pr[B正确] - 1/2| = |1 - 1/2| = 1/2？实际上如果A总是正确，则B正确概率为1，语义安全优势定义为|Pr[输出1|b=1] - Pr[输出1|b=0]|，这里B输出1当且仅当A(c)=1，而c来自m1时A(c)=1，来自m0时A(c)=0，所以Pr[输出1|b=1]=1，Pr[输出1|b=0]=0，优势=1。因此答案为1。
...
```c = k ⊕ m0，其中k均匀随机。由于k与m0独立，且异或一个固定值仍为均匀分布，故c在{0,1}^n上均匀。
...
```对于任意攻击者A，Pr[A(c)=1]在EXP(0)和EXP(1)中相等，因此
Adv_SS[A, OTP] = |Pr[A(k⊕m0)=1] - Pr[A(k⊕m1)=1]| = 0。
