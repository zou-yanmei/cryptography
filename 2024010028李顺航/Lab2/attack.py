...
target_cipher = ciphertexts[-1]  # 提取目标密文
...
for pos in range(msg_len):
    ...
    for i in range(num_ciphertexts):
        if pos >= len(ciphertexts[i]):
            continue
        # 假设第 i 条密文该位置明文为空格 (0x20)，猜测密钥字节
        guess_key = ciphertexts[i][pos] ^ 0x20
        count = 0
        for j in range(num_ciphertexts):
            if i == j or pos >= len(ciphertexts[j]):
                continue
            # 用猜测密钥解密第 j 条密文，检查是否为字母
            decrypted_char = ciphertexts[j][pos] ^ guess_key
            if chr(decrypted_char).isalpha():
                count += 1
...
# 输出结果
print("=" * 60)
print("还原密钥流 (十六进制):", binascii.hexlify(key_stream).decode())
print("=" * 60)
print("解密后明文:")
