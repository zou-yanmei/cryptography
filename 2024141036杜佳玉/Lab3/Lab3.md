...
**答案：** _______6c73d5240a948c86981bc2946475736b____
...
**答案：不安全。因为存在高效测试 A 使得优势不可忽略，PRG 可被区分。**
...
**答案：对任意明文 m、密文 c，Pr_k [E (k,m)=c] = 1/|K|，与 m 无关。​**
...
**答案：恰好存在 1 个密钥 k = m XOR c 满足 E (k,m)=c。**
...
**答案：对任意 m，Pr [E (k,m)=c] = 1/|K|，与明文 m 无关，满足完美保密性定义，故 OTP 具有完美保密性。**
...
**答案：若算法 A 总能从密文得到 LSB (m)，则攻击者可在语义安全游戏中以显著优势区分 m0、m1，因此该加密方案不是语义安全的。**
...
**答案：选择 m0、m1 仅最低有效位不同，其余位相同。B 收到密文 c 后，调用 A (c) 得到 LSB (m)，以此作为对 b 的猜测并输出。**
...
**答案：k 均匀随机，m0 固定，c = k XOR m0 仍在 {0,1}^n 上均匀分布。**
...
**答案：同理，k 均匀随机，m1 固定，c = k XOR m1 也均匀分布。**
...
**答案：EXP (0) 与 EXP (1) 中密文分布完全相同，均为均匀分布。**
...
**答案：由于两个实验密文分布相同，对任意攻击者 A，两个概率相等，差值为 0，因此语义安全优势为 0，OTP 语义安全。**
