...
**答案：** ____6c73d5240a948c86981bd194815c_______
...
=\left|\Pr_{k}[A(G(k))=1] - \Pr_{r}[A(r)=1]\right|
```
...
```
接近 1：说明 A 能非常有效地区分 PRG 输出与真随机串，PRG 不安全。
接近 0：说明 A 无法有效区分，PRG 对该测试安全。
```
...
\mathrm{Adv}=\left|\frac23-\frac12\right|=\frac16\approx 0.1667
...
```
不安全。因为存在高效区分器 A 使得优势显著大于 0，不满足安全 PRG 定义。
```
...
对任意 m_0,m_1\in\mathcal{M} 和任意 c\in\mathcal{C}：
...
```
对任意 m,c，满足 E(k,m)=c 的密钥唯一。密钥均匀随机，故
\Pr[E(k,m)=c]=\frac{1}{|\mathcal{K}|}与 m 无关，满足完美保密定义。
```
...
```
若能从密文稳定得到 LSB(m)，则攻击者可区分对 m_0,m_1 的加密，违背语义安全。
```
...
```
选 m_0 满足 LSB=0，m_1 满足 LSB=1。
收到 c 后用 A 得到 LSB(m)，若为 0 则猜 b=0，否则猜 b=1。
```
...
```
k 均匀且与 m_0 独立，故 c=k\oplus m_0 均匀。
```
...
```
分布相同，故概率相等，差值为 0，优势恒为 0，因此 OTP 语义安全。
```
