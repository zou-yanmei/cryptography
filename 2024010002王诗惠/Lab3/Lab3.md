...
**答案：** $$\mathrm{Adv}_{\mathrm{PRG}}[A, G] = \left| \Pr_{k \xleftarrow{R} K}[A(G(k)) = 1] - \Pr_{r \xleftarrow{R} \{0,1\}^n}[A(r) = 1] \right|$$
...
优势接近 1：统计测试 $A$ 能够以很高的概率区分 $G(k)$ 与真随机串，说明 $G$ 不安全。  
优势接近 0：$A$ 无法有效区分，$G$ 相对于 $A$ 是安全的。

**(c)** 若 $A(x) = 0$（即 $A$ 对任何输入都输出 0），求 $\mathrm{Adv}_{\mathrm{PRG}}[A, G]$ 的值。

**答案：**

若 $A(x) \equiv 0$，则 $\Pr[A(G(k))=1] = 0$，$\Pr[A(r)=1] = 0$，故优势为 $0$。
...
$$\mathrm{Adv}_{\mathrm{PRG}}[A, G] = \left|\frac{2}{3} - \frac{1}{2}\right| = \frac{1}{6} \approx 0.1667$$

**(d)** 该 PRG 是否安全？为什么？

**答案：**

不安全。因为优势 $\frac{1}{6}$ 显著大于 0，存在有效的统计测试能够以非可忽略的优势区分输出与真随机串。
...
完美保密性定义：对于任意明文 $m \in \mathcal{M}$ 和密文 $c \in \mathcal{C}$，有  $$\Pr_{k \xleftarrow{R} K}[E(k,m) = c] = \frac{1}{|\mathcal{M}|}$$

**(b)** 对于 OTP，证明对任意 $m \in \mathcal{M}$ 和 $c \in \mathcal{C}$，恰好存在多少个密钥 $k$ 使得 $E(k, m) = c$？

**答案：**

对于 OTP，$E(k,m) = m \oplus k$。给定 $m$ 和 $c$，方程 $m \oplus k = c$ 有唯一解 $k = m \oplus c$，因此恰好存在 **1** 个密钥。

**(c)** 利用 (a) 和 (b) 的结论，完成 OTP 具有完美保密性的证明。

**答案：**

由于密钥均匀随机，$\Pr[E(k,m)=c] = \Pr[k = m \oplus c] = \frac{1}{|K|}$。而 $|K| = |\mathcal{M}|$，所以等于 $\frac{1}{|\mathcal{M}|}$，满足完美保密性定义。
...
若存在算法 $A$ 总能从密文 $c$ 中正确输出 $\mathrm{LSB}(m)$，则加密方案不是语义安全的。因为语义安全要求攻击者无法区分两个不同消息的加密，而 $A$ 可以轻易区分 $\mathrm{LSB}$ 不同的消息。
...
攻击者 $B$ 选择两条消息 $m_0, m_1$，使得 $\mathrm{LSB}(m_0)=0$，$\mathrm{LSB}(m_1)=1$。$B$ 将 $(m_0, m_1)$ 发送给挑战者，收到挑战密文 $c$。$B$ 计算 $b' = A(c)$，若 $b' = 0$ 则猜测 $b=0$（即加密的是 $m_0$），否则猜测 $b=1$。由于 $A$ 总是正确输出 $\mathrm{LSB}$，$B$ 总能正确猜出 $b$。

**(c)** 计算 $\mathrm{Adv}_{\mathrm{SS}}[B, \mathbb{E}]$ 的值。

**答案：**

$\mathrm{Adv}_{\mathrm{SS}}[B, \mathbb{E}] = \left| \Pr[\text{EXP}(0)=1] - \Pr[\text{EXP}(1)=1] \right| = |0 - 1| = 1$。
...
在 EXP(0) 中，$c = k \oplus m_0$，$k$ 均匀随机，则 $c$ 在 $\{0,1\}^n$ 上均匀分布。

**(b)** 证明在 EXP(1) 中，密文 $c = k \oplus m_1$ 在 $\{0,1\}^n$ 上也是均匀分布的。

**答案：**

在 EXP(1) 中，$c = k \oplus m_1$，同样 $k$ 均匀随机，$c$ 也均匀分布。

**(c)** 由 (a) 和 (b) 说明 EXP(0) 和 EXP(1) 中密文的分布是相同的（identical distributions）。

**答案：**

由 (a) 和 (b) 知，两种实验下密文 $c$ 的分布完全相同，均为 $\{0,1\}^n$ 上的均匀分布。
...
对于任意攻击者 $A$，$\Pr[A(c)=1]$ 在两种实验下相等，因此  
$$\mathrm{Adv}_{\mathrm{SS}}[A, \mathrm{OTP}] = \left| \Pr[A(k \oplus m_0)=1] - \Pr[A(k \oplus m_1)=1] \right| = 0$$
